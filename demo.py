...
class Application(tk.Frame):
    ...
    def generate_figure(self):
        ...
        dlatents = self.Gs.components.mapping.run(self.latents, [label])
        dlatent_avg = self.Gs.get_var('dlatent_avg')
        self.dlatents = (dlatents - dlatent_avg) * np.reshape(self.truncation_psi_var.get(), [-1, 1, 1]) + dlatent_avg

        images = self.Gs.run(self.latents, [label], truncation_psi=self.truncation_psi_var.get(), randomize_noise=False, output_transform=fmt)
    ...
    def create_widgets(self):
        ...
        self.truncation_psi_var.set(1.0)
        self.style_var = tk.IntVar()
        self.style_var.set(2)
        ...
        self.color_radio_button = tk.Radiobutton(self, value=2, variable=self.style_var, text='Color')

        # Settings
        self.animation_checkbox = tk.Checkbutton(self, text='Animation', variable=self.is_animation)
        self.truncation_psi_scale = tk.Scale(self, orient="horizontal", variable=self.truncation_psi_var, from_=0.0, to=1.0, resolution=0.1)
